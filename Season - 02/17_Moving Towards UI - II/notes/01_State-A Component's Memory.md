...
> ⚠️ **Warning**
>
> - Hooks (functions starting with `use`) must be called only at the **top level** of React components or inside **custom Hooks**.
> - Do **not** call Hooks inside **conditions, loops, or nested functions**.
> - Even though Hooks are functions, treat them as **declarations of your component’s needs** that must run in the same order on every render.
> - Use Hooks at the **top of your component**, similar to how **imports are placed at the top of a file**.
...
- It is a good idea to have multiple state variables if their state is unrelated, like index and showMore in this example. But if you find that you often change two state variables together, it might be easier to combine them into one. 
...
(We will go deep dive into this later on , how to choose perfect state structure)
...
# 💡 **How `useState` Knows Which State to Return**

- You might notice that `useState` does not receive any identifier telling it which state variable it belongs to.  

- There is no argument like `"count"` or `"name"` passed to it. So how does React know which state to return?

- React **does not parse your code or use any magic**. Instead, Hooks rely on a **consistent call order** during every render of a component.

## Key Idea

- Hooks must always be called in the **same order** on every render.  

- This is why React enforces the rule that Hooks should only be called **at the top level of a component** and **not inside conditions, loops, or nested functions**.

- Because of this rule, React can reliably match each Hook call with its stored state.


### How It Works Internally

- React stores the state of each component in an **array of state pairs**.
- Before rendering a component, React sets an internal **index pointer to `0`**.
- Every time `useState` is called:
  - React returns the **state pair at the current index**.
  - Then it **increments the index**.
- On the next render, React resets the index and repeats the same sequence.

- Because the **order of Hook calls stays the same**, React can correctly associate each `useState` call with its corresponding state.
...
- There are `two main usage rules` the React core team stipulates you need to follow to use hooks which they outline in the hooks proposal documentation.
  ...
  - Only Call Hooks from React Components(in old versions :: Functions).
...
> **NOTE** **:** Please note this is speculation and only one possible way of implementing the API to show how you might want to think about it. This is not necessarily how the API works internally.
...
- Let’s annotate how this might work internally within React.
- The following would work within `the execution context` for rendering a particular component. That means that `the data stored here lives one level outside of the component being rendered`. 
- This `state is not shared with other components but it is maintained in a scope that is accessible to subsequent rendering of the specific component`.

**1) Initialisation:**
- Create two empty arrays: `setters` and `state`.
- Set a cursor to 0

![initialization](../photos/useState1.webp)
...
![Event Handling](../photos/useState4.webp)
